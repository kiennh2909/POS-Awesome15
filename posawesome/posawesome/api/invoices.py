...
def update_invoice(data):
	...
	if data.get("name"):
		...
		if data.get("pos_profile"):
			...
			if hasattr(pos_profile_doc, 'taxes_and_charges') and pos_profile_doc.taxes_and_charges:
				...
				# Don't modify POS Profile directly to avoid LinkValidationError
				# Instead, we'll override in invoice document after set_missing_values
	...
	# CRITICAL FIX: Override taxes_and_charges in invoice document to prevent POS Profile 8% tax override
	if data.get("pos_profile"):
		pos_profile_doc = frappe.get_doc("POS Profile", data.get("pos_profile"))
		if hasattr(pos_profile_doc, 'taxes_and_charges') and pos_profile_doc.taxes_and_charges:
	...
	# Always rely on item-level tax calculation when items have item_tax_template
	# ERPNext will automatically create appropriate tax entries during calculate_taxes_and_totals()
	log.info(f"[UPDATE_INVOICE] ℹ️ Relying on item-level tax calculation for all invoices with item_tax_template")
	...
	if hasattr(invoice_doc, 'taxes') and invoice_doc.taxes:
		...
		if is_inclusive:
			...
			# Update invoice totals after adjusting net_amounts
			log.info(f"[UPDATE_INVOICE] 🧾 Updating invoice totals after inclusive tax adjustment")
			invoice_doc.total = sum(flt(item.net_amount) for item in invoice_doc.items)
			invoice_doc.net_total = invoice_doc.total
			log.info(f"[UPDATE_INVOICE] 🧾 Updated invoice totals: total={invoice_doc.total:.2f}, net_total={invoice_doc.net_total:.2f}")
