		...
		<v-dialog v-model="dialog" max-width="600px" persistent>
			<v-card>
				...
				<v-card-text>
					...
					<v-alert v-if="!isPosProfileValid" type="error" outlined class="mb-4">
					...
					<v-alert type="info" outlined class="mb-4">
						...
						<div class="mt-2 text-caption">Nhập thông tin để bắt đầu cuộn hóa đơn thuế mới</div>
					</v-alert>
					...
					<v-card v-if="isFormValid" outlined class="mt-4">
						...
						<v-card-text class="pt-0">
							<div class="d-flex align-center">
								...
								<span class="text-caption text--secondary"> (Cuộn mới) </span>
							</div>
						</v-card-text>
					</v-card>
				</v-card-text>
				...
				<v-card-actions>
					<v-spacer></v-spacer>
					<v-btn text @click="closeDialog" :disabled="loading"> Hủy </v-btn>
					<v-btn color="primary" @click="submitTaxRoll" :disabled="!isFormValid" :loading="loading">
						Bắt Đầu Cuộn Mới
					</v-btn>
				</v-card-actions>
			</v-card>
	...
	data() {
		return {
			...
			rules: {
				...
				prefixFormat: (value) =>
					/^[A-Z]{2}$/.test(value) || "Prefix phải là đúng 2 ký tự in hoa (VD: PW, BZ, CZ)",
